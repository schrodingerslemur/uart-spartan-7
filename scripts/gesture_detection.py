...
# Configuration
SERIAL_PORT = 'COM8'  # Change this to your desired port
BAUD_RATE = 9600
CAMERA_INDEX = 0
...
class HandGestureDetector:
    ...
    def send_uart_data(self, left_y, right_y):
        """Send Y positions via UART"""
        if self.ser and self.ser.is_open:
            try:
                # Format: "L:xxx,R:xxx\n"
                data = f"L:{left_y:03d},R:{right_y:03d}\n"
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                if detection_result.hand_landmarks and detection_result.handedness:
                    for hand_landmarks, handedness in zip(
                        ...
                        if hand_label == "Left":
                            ...
                            right_y = y_pos
                            # Draw on right frame
                            self.draw_landmarks(right_frame, hand_landmarks, w//2, h)
                ...
                cv2.putText(combined_frame, f"UART: L:{left_y:03d} R:{right_y:03d}", 
